  ...
  <aside data-sidebar class="fixed inset-y-0 left-0 z-50 flex w-[280px] -translate-x-full flex-col bg-slate-950 text-white shadow-2xl transition-transform duration-300 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0">
    <div class="flex h-20 items-center justify-between border-b border-white/10 px-5">
      <a href="#/dashboard" class="flex items-center gap-3" aria-label="RetraLabs Edu">
        <span class="grid size-11 place-items-center rounded-2xl bg-gradient-to-br from-brand-500 to-accent-500 font-black shadow-lg shadow-brand-950/40">R</span>
        <span>
      ...
      <button type="button" data-sidebar-close class="rounded-xl p-2 text-slate-400 transition hover:bg-white/10 hover:text-white lg:hidden" aria-label="Tutup sidebar">
        <i data-lucide="X" class="size-5"></i>
      </button>
    </div>

    <div class="flex-1 overflow-y-auto px-4 py-5">
      <div class="mb-5 rounded-2xl border border-brand-500/20 bg-brand-500/10 p-4">
        ...
        <p class="mt-2 text-sm font-bold text-white">2026/2027 · Ganjil</p>
          ...
          <div>
            ...
            <div class="space-y-1">
              ${group.items.map((item) => `
                <a href="#${item.path}" class="nav-link ${activePath === item.path ? 'active' : ''}" data-nav-link>
    ...
    <div class="border-t border-white/10 p-4">
      <div class="mb-3 rounded-2xl bg-gradient-to-br from-brand-600/30 to-accent-500/10 p-4">
        <div class="flex items-center justify-between">
          <span class="badge bg-white/10 text-brand-200">Paket Sekolah</span>
          <i data-lucide="Crown" class="size-4 text-amber-300"></i>
        </div>
        <p class="mt-2 text-xs text-slate-300">Semua fitur aktif sampai 30 Juni 2027.</p>
      </div>
      <div class="flex items-center gap-3 rounded-2xl p-2 transition hover:bg-white/5">
        <div class="grid size-10 place-items-center rounded-xl bg-white/10 text-sm font-black">${currentUser.initials}</div>
